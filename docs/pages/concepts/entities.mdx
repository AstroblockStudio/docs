...
The galaxy map is filled with entities which are celestial objects defined by their `x` & `y` positions as well as a radius `r`.
...
As of writting 5070 (of 30k) entities (stars) have been discovered by players ! You can see the trail some of them are leaving. All of which is permanently onchain.

<img src="/partial-universe.png" alt="5k image" style={{objectFit: "contain"}} />

### Tokens

All entities are also represented onchain by a ERC721 tokens that are all stored in the map contract for now. You can see the tokens being minted in the OpenSea collection.
