...
In order to create interactions. A zone around every entity is simply defined by half of the entity's radius. This zone is defined as the entity's **vicinity**.
It can be checked onchain & used by any external contracts.

<img src="/anchoring.png" alt="anchoring image" style={{objectFit: "contain"}} />

:::info
It is used for **star discovery**. When a player submits a star discovery request, the map contract checks if the player is in the vicinity of the star. If they are (among other checks), the star is added to the map.
::: 
...
- `onMovingTo()` : Called when a player moves into the vicinity of the entity.
- `onLeave()` : Called when a player moves out of the vicinity of the entity.
- Hook contract can also call `isMoving()` getter in order to know if a player has arrive or is in transit to it.
 
<img src="/entity-module.png" alt="entity module image" style={{objectFit: "contain"}} />
